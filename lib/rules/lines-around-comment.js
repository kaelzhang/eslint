...
module.exports = {
    ...
    create: function(context) {
        ...
        /**
         * Returns whether or not two tokens or nodes are on the same line.
        ...
        function codeAroundComment(node) {
            ...
            if (token && areOnSameLine(token, node)) {
                return true;
            }
            ...
            if (token && areOnSameLine(node, token)) {
                return true;
            }
        ...
        function checkForEmptyLine(node, opts) {
            ...
            if (!exceptionStartAllowed && before && !lodash.includes(commentAndEmptyLines, prevLineNum) &&
                    !(isCommentNodeType(previousTokenOrComment) && areOnSameLine(previousTokenOrComment, node))) {
            ...
            if (!exceptionEndAllowed && after && !lodash.includes(commentAndEmptyLines, nextLineNum) &&
                    !(isCommentNodeType(nextTokenOrComment) && areOnSameLine(node, nextTokenOrComment))) {
                context.report({
                    node: node,
